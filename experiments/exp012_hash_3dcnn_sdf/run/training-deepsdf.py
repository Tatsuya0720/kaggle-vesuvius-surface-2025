...
points_path = glob("../../../input/04_sdf_dataset_by10/**/points.npy")
...
optimizer = torch.optim.Adam(
    [
        # Encoderは慎重に学習 (例: 1e-5)
        {"params": model.encoder.parameters(), "lr": cfg.deepsdf_initial_lr * 0.1},
        # Decoder (SIREN) は標準の速度で学習 (例: 1e-4)
        # ※PosEncがある場合、それもここに含める
        {"params": model.decoder_net.parameters(), "lr": cfg.deepsdf_initial_lr},
        {"params": model.final_layer.parameters(), "lr": cfg.deepsdf_initial_lr},
    ]
)
...
for epoch in range(cfg.n_epochs):
    ...
    # last-epoch save
    os.makedirs("../outputs/models/", exist_ok=True)
    torch.save(model.state_dict(), "../outputs/models/deepsdf_encoder_last.pth")
    print(f"Model saved at last epoch {epoch}.")
